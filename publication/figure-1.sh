...
subsample-reads map --in-bam $MAP_BAM --contig $CHR --start $START  --end $END --interval-count 30 --bed $BED && \
subsample-reads sample --seed $SEED --in-bam $IN_BAM --bed $BED --out-bam $OUT_BAM && \
subsample-reads plot --no-det --out-bam $OUT_BAM --bed $BED --out-plt $OUT_PLT
...
subsample-reads plot --no-det --out-bam $SAMTOOLS_OUT_BAM --bed $BED --out-plt $SAMTOOLS_OUT_PLT